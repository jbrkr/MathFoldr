...
from dataaccess import conceptlink
...
async def load_concept_links(dataset_id):
    print("Loading concept Links...")

    # Dataset
    # dataset = await datasets.get(dataset_id)

    # Load csv file with concept linking database
    df = pd.read_csv('data/concepts.csv')

    tasks = []

    # Delete all rows from conceptlinks
    ...
    for row in df.iterrows():
        print(row)
        tasks.append(conceptlink.create(alt_id=str(row[1]['wiki_id']),
                                    dataset_id=dataset_id,
                                    concept_name=row[1]['name'],
